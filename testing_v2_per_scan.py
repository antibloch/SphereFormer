...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    val_data = SemanticKITTI_Pathetic(data_path=args.data_root, 
        ...
        test_path_label='refined_data/labels',
    )
...
def validate_distance(val_loader, model, criterion):
    ...
    for batch_n, batch_data in enumerate(val_loader):
        ...
        batch_n_revised = int(fil_name[0])
        ...
        labels_np = ref_labels.cpu().numpy().squeeze(-1)  
        ...
        for label in range(num_classes+1):
            pred_colors[segmentation_np == label] = sup_colors[label]


        for label in range(num_classes+1):
            ref_colors[labels_np == label] = sup_colors[label]
        ...
        view_frame(points_np,  pred_colors, ref_colors,  legend_patches, "all_results", batch_n_revised)
        ...
        overall_data = [f'{fil_name[0]}.ply'] + classy_iou
    ...
    print("===========Mean Scores=====================")
    for i in range(num_classes):
        print(f"Class {class_names[i]}: {per_classes_ious[i]:.4f}" if per_classes_ious[i] != 'nan' else f"Class {class_names[i]}: N/A")
    print("-------------------------------------------")
